...
layui.define(['jquery'], function (exports) {
    ...
    let REGEXP_DATA_URL_HEAD = /^data:.*,/;
    let REGEXP_USERAGENT = /(Macintosh|iPhone|iPod|iPad).*AppleWebKit/i;
    let navigator = typeof window !== 'undefined' ? window.navigator : null;
    let IS_SAFARI_OR_UIWEBVIEW = navigator && REGEXP_USERAGENT.test(navigator.userAgent);
    ...
    function proxy(fn, context) {
        ...
        return function () {
            for (let _len2 = arguments.length, args2 = Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
    ...
    function getTransform(options) {
        let transforms = [];
        let translateX = options.translateX;
        let translateY = options.translateY;
    ...
    function getRotatedSizes(data, isReversed) {
        let deg = Math.abs(data.degree) % 180;
        let arc = (deg > 90 ? 180 - deg : deg) * Math.PI / 180;
    ...
    function getSourceCanvas(image, data, options) {
        ...
        let dstX = 0;
        let dstY = 0;
        let dstWidth = data.naturalWidth;
        let dstHeight = data.naturalHeight;
        let rotate = data.rotate;
        let scaleX = data.scaleX;
        let scaleY = data.scaleY;
        let scalable = isNumber(scaleX) && isNumber(scaleY) && (scaleX !== 1 || scaleY !== 1);
        let rotatable = isNumber(rotate) && rotate !== 0;
        let advanced = rotatable || scalable;
    ...
    function dataURLToArrayBuffer(dataURL) {
        let base64 = dataURL.replace(REGEXP_DATA_URL_HEAD, '');
    ...
    let render = {
        ...
        initCanvas: function initCanvas() {
            let self = this;
            let viewMode = self.options.viewMode;
            let container = self.container;
            let containerWidth = container.width;
            let containerHeight = container.height;
            let image = self.image;
            let imageNaturalWidth = image.naturalWidth;
            let imageNaturalHeight = image.naturalHeight;
            let is90Degree = Math.abs(image.rotate) % 180 === 90;
            let naturalWidth = is90Degree ? imageNaturalHeight : imageNaturalWidth;
            let naturalHeight = is90Degree ? imageNaturalWidth : imageNaturalHeight;
        ...
        limitCanvas: function limitCanvas(isSizeLimited, isPositionLimited) {
            ...
            let aspectRatio = canvas.aspectRatio;
            let cropBox = self.cropBox;
            let cropped = self.cropped && cropBox;
            ...
            if (isPositionLimited) {
                if (viewMode) {
                    let newCanvasLeft = containerWidth - canvas.width;
                    let newCanvasTop = containerHeight - canvas.height;
        ...
        renderCanvas: function renderCanvas(isChanged) {
            ...
            if (self.rotated) {
                ...
                let rotated = getRotatedSizes({
                    width: image.width,
                    height: image.height,
                    degree: rotate
                });
                let aspectRatio = rotated.width / rotated.height;
                let isSquareImage = image.aspectRatio === 1;
        ...
        initCropBox: function initCropBox() {
            ...
            let autoCropArea = Number(options.autoCropArea) || 0.8;
            let cropBox = {
                width: canvas.width,
                height: canvas.height
            };
    ...
    let preview = {
        ...
        resetPreview: function resetPreview() {
            this.$preview.each(function (i, element) {
                let $this = $(element);
                let data = $this.data(DATA_PREVIEW);
        ...
        preview: function preview() {
            ...
            let canvas = self.canvas;
            let cropBox = self.cropBox;
            let cropBoxWidth = cropBox.width;
            let cropBoxHeight = cropBox.height;
            let width = image.width;
            let height = image.height;
            let left = cropBox.left - canvas.left - image.left;
            let top = cropBox.top - canvas.top - image.top;
    ...
    // Globals
    let PointerEvent = typeof window !== 'undefined' ? window.PointerEvent : null;

    // Events
    let EVENT_POINTER_DOWN = PointerEvent ? 'pointerdown' : 'touchstart mousedown';
    let EVENT_POINTER_MOVE = PointerEvent ? 'pointermove' : 'touchmove mousemove';
    let EVENT_POINTER_UP = PointerEvent ? ' pointerup pointercancel' : 'touchend touchcancel mouseup';
    let EVENT_WHEEL = 'wheel mousewheel DOMMouseScroll';
    let EVENT_DBLCLICK = 'dblclick';
    let EVENT_RESIZE = 'resize';
    let EVENT_CROP_START = 'cropstart';
    let EVENT_CROP_MOVE = 'cropmove';
    let EVENT_CROP_END = 'cropend';
    let EVENT_CROP = 'crop';
    let EVENT_ZOOM = 'zoom';
    ...
    let REGEXP_ACTIONS = /^(e|w|s|n|se|sw|ne|nw|all|crop|move|zoom)$/;
    ...
    let handlers = {
        resize: function resize() {
            ...
            let $container = self.$container;
            let container = self.container;
            let minContainerWidth = Number(options.minContainerWidth) || 200;
            let minContainerHeight = Number(options.minContainerHeight) || 100;

            if (self.disabled || container.width === minContainerWidth || container.height === minContainerHeight) {
                return;
            }

            let ratio = $container.width() / container.width;
        ...
        wheel: function wheel(event) {
            ...
            let ratio = Number(self.options.wheelZoomRatio) || 0.1;
    ...
    let ACTION_SOUTH = 's';
    let ACTION_NORTH = 'n';
    let ACTION_SOUTH_EAST = 'se';
    let ACTION_SOUTH_WEST = 'sw';
    let ACTION_NORTH_EAST = 'ne';
    let ACTION_NORTH_WEST = 'nw';

    function getMaxZoomRatio(pointers) {
        let pointers2 = $.extend({}, pointers);
        let ratios = [];
        ...
        $.each(pointers, function (pointerId, pointer) {
            ...
            $.each(pointers2, function (pointerId2, pointer2) {
                let x1 = Math.abs(pointer.startX - pointer2.startX);
                let y1 = Math.abs(pointer.startY - pointer2.startY);
                let x2 = Math.abs(pointer.endX - pointer2.endX);
                let y2 = Math.abs(pointer.endY - pointer2.endY);
    ...
    let change = {
        change: function change(e) {
            ...
            let options = self.options;
            let pointers = self.pointers;
            let pointer = pointers[objectKeys(pointers)[0]];
            let container = self.container;
            let canvas = self.canvas;
            let cropBox = self.cropBox;
            let action = self.action;
    ...
    let methods = {
        ...
        zoomTo: function zoomTo(ratio, _event) {
            ...
            let naturalWidth = canvas.naturalWidth;
            let naturalHeight = canvas.naturalHeight;
            ...
            if (ratio >= 0 && self.ready && !self.disabled && options.zoomable) {
                ...
                if (originalEvent) {
                    let offset = self.$cropper.offset();
                    let center = pointers && objectKeys(pointers).length ? getPointersCenter(pointers) : {
        ...
        setCropBoxData: function setCropBoxData(data) {
            let self = this;
            let cropBox = self.cropBox;
            let aspectRatio = self.options.aspectRatio;
        ...
        getCroppedCanvas: function getCroppedCanvas(options) {
            ...
            let canvasWidth = Math.floor(scaledWidth || originalWidth);
            let canvasHeight = Math.floor(scaledHeight || originalHeight);
            ...
            let parameters = function () {
                let source = getSourceCanvas(self.$clone[0], self.image, options);
                let sourceWidth = source.width;
                let sourceHeight = source.height;
                let canvasData = self.canvas;
                let params = [source];
    ...
    let CLASS_HIDDEN = 'cropper-hidden';
    let REGEXP_DATA_URL = /^data:/;
    let REGEXP_DATA_URL_JPEG = /^data:image\/jpeg;base64,/;
    ...
    let Cropper = function () {
        ...
        _createClass(Cropper, [{
            ...
            value: function read(arrayBuffer) {
                let self = this;
                let options = self.options;
                let orientation = getOrientation(arrayBuffer);
                let image = self.image;
                let rotate = 0;
                let scaleX = 1;
                let scaleY = 1;
            ...
            value: function build() {
                ...
                let $cropper = $(TEMPLATE);
                let $cropBox = $cropper.find('.cropper-crop-box');
                let $face = $cropBox.find('.cropper-face');
    ...
    let NAMESPACE = 'cropper';
    let OtherCropper = $.fn.cropper;
    ...
    $.fn.cropper = function jQueryCropper(option) {
        ...
        this.each(function (i, element) {
            ...
            if (!data) {
                ...
                let options = $.extend({}, $this.data(), $.isPlainObject(option) && option);
